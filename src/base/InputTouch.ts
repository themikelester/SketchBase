//----------------------------------------------------------------------------------------------------------------------
// Notes: Low-level listener for touch events. This is the only class that should be making addEventListener calls. Any
//        module that needs touch events should subscribe to events from this class.
//
//        // Subscribe to an event:
//        touchDevice.on( 'touchstart', e => {
...
// Passive listeners don't count as user events for fullscreening.
// See https://stackoverflow.com/questions/42945378/full-screen-event-on-touch-not-working-on-chrome
...
export class TouchWrapper {
    ...
    */
    ...
    private getTouchTargetCoords( touch: Touch ): TouchCoords {
        let totalOffsetX = 0;
        let totalOffsetY = 0;
        let target = touch.target as Node;
...
export class TouchDevice extends EventDispatcher {
    ...
    */
    constructor( element: Element ) {
        ...
        this.onStart = ( e: Event ) => {
            e.preventDefault(); this.fire( 'touchstart', new TouchEventWrapper( this, e as TouchEvent ) );
        };

        this.onEnd = ( e: Event ) => {
            e.preventDefault(); this.fire( 'touchend', new TouchEventWrapper( this, e as TouchEvent ) );
        };

        this.onCancel = ( e: Event ) => {
            this.fire( 'touchcancel', new TouchEventWrapper( this, e as TouchEvent ) );
        };

        this.onMove = ( e: Event ) => {
            e.preventDefault(); this.fire( 'touchmove', new TouchEventWrapper( this, e as TouchEvent ) );
        };
